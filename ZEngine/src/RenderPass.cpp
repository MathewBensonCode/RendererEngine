...
namespace ZEngine::Rendering::Renderers::RenderPasses
{
            ...
            switch (input.Type)
            {
                case UNIFORM_BUFFER_SET:
                {
                    ...
                    for (auto& uniform_buffer : uniform_buffer_collection)
                    {
                        ...
                        write_descriptor_set_collection.emplace_back(
    ...
    void RenderPass::SetInput(std::string_view key_name, const Ref<Textures::TextureArray>& textures)
    {
        ...
        if (validity_output.first)
        {
            ...
            m_input_collection.emplace_back(
                PassInput{
                    .Set = binding_spec.Set, .Binding = binding_spec.Binding, .DebugName = binding_spec.Name, .Type = PassInputType::TEXTURE_ARRAY, .Input = {textures.get()}});
    ...
    void RenderPass::SetInput(std::string_view key_name, const Ref<UniformBuffer>& buffer)
    {
        ...
        if (validity_output.first)
        {
            ...
            m_input_collection.emplace_back(
                PassInput{
                    .Set = binding_spec.Set, .Binding = binding_spec.Binding, .DebugName = binding_spec.Name, .Type = PassInputType::UNIFORM_BUFFER, .Input = {buffer.get()}});
    ...
    void RenderPass::SetInput(std::string_view key_name, const Ref<StorageBuffer>& buffer)
    {
        ...
        if (validity_output.first)
        {
            ...
            m_input_collection.emplace_back(
                PassInput{
                    .Set = binding_spec.Set, .Binding = binding_spec.Binding, .DebugName = binding_spec.Name, .Type = PassInputType::STORAGE_BUFFER, .Input = {buffer.get()}});
